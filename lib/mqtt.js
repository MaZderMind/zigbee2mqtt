...
class MQTT extends events.EventEmitter {
    ...
    async connect() {
        ...
        if (mqttSettings.user && mqttSettings.password) {
        ...
        else if (mqttSettings.googleJwt) {
            function createSignedToken() {
                ...
                const token = {
                  ...
                  aud: mqttSettings.googleJwt.projectId,
                };
                const privateKey = fs.readFileSync(mqttSettings.googleJwt.privateKeyFile);
                return jwt.sign(token, privateKey, {algorithm: mqttSettings.googleJwt.algorithm});
            }

            options.username = 'unused';
            options.password = createSignedToken();

            // Setup a Re-Key Interval
            const interval = 15 * 60 * 1000; // 15 minutes
            this.connectionTimer = setInterval(() => {
                logger.info("Started Google JWT Re-Key");
                options.password = createSignedToken();

                const newClient = mqtt.connect(mqttSettings.server, options);
                newClient.on('connect', () => {
                    const oldClient = this.client;
                    this.client = newClient;
                    oldClient.end();

                    this.publish('bridge/re-key', 'successful', {qos: 0});
                    logger.info("Finished Google JWT Re-Key");
                });
            }, interval);
        }
