...
class MQTT extends events.EventEmitter {
    ...
    async connect() {
        ...
        this.connectionTimer = setInterval(() => {
            logger.error('IN TIMER reconnecting ' + this.client.reconnecting + ' connected ' + this.client.connected);
            if (this.client.reconnecting) {
                logger.error('Not connected to MQTT server! TIMER');
            }
        }, interval);

        logger.error('GOT HERE 4');

        return new Promise((resolve) => {
            logger.error('GOT HERE 5');
            this.client = mqtt.connect(mqttSettings.server, options);

            logger.error('GOT HERE 6');

            this.client.on('connect', () => {
                logger.error('GOT HERE 7');
                logger.info('Connected to MQTT server');
                this.publish('bridge/state', 'online', {retain: true, qos: 0});
                resolve();
            });

            logger.error('GOT HERE 8');
    ...
    async publish(topic, payload, options, base=settings.get().mqtt.base_topic) {
        ...
        if (!this.isConnected()) {
            logger.error(`Not connected to MQTT server! (MESSAGE)`);
